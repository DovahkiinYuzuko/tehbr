...
function extractAlignment(tagStr: string): Alignment | null {
  const styleMatch = /style\s*=\s*["'][^"']*text-align\s*:\s*(left|center|right)[^"']*["']/i.exec(tagStr);
  if (styleMatch?.[1]) {
    return styleMatch[1].toLowerCase() as Alignment;
  }

  const alignMatch = /align\s*=\s*["']?(left|center|right)["']?/i.exec(tagStr);
  if (alignMatch?.[1]) {
    return alignMatch[1].toLowerCase() as Alignment;
  }

  return null;
}

export function parseHTML(content: string): TehbrIR {
